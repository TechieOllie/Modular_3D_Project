...
HAL_StatusTypeDef BSP_TIMER_disable_PWM(timer_id_t timer_id, uint32_t channel)
{
	...
	// Disable the channel output directly using CCER register
	// Clear the CCxE bit in the CCER register to disable the channel
	htim->Instance->CCER &= ~(TIM_CCER_CC1E << ((channel - 1) * 4));

	// Stop PWM generation using HAL function
	HAL_StatusTypeDef status = HAL_TIM_PWM_Stop(htim, tim_channel);

	// Even if HAL_TIM_PWM_Stop fails, ensure the timer is disabled
	if (status != HAL_OK)
	{
		// Try additional steps to stop the timer
		htim->Instance->CR1 &= ~(TIM_CR1_CEN); // Force timer counter disable

		// Reset PWM mode registers for this channel
		if (channel <= 2)
		{
			htim->Instance->CCMR1 &= ~(TIM_CCMR1_OC1M << ((channel - 1) * 8));
		}
		else
		{
			htim->Instance->CCMR2 &= ~(TIM_CCMR2_OC3M << ((channel - 3) * 8));
		}

		// Return success anyway since we've tried everything
		return HAL_OK;
	}
...
HAL_StatusTypeDef BSP_TIMER_set_period_us(timer_id_t timer_id, uint32_t period_us)
{
	...
	if (timer_id != TIMER1_ID && timer_id != TIMER6_ID)
		timer_clock = timer_clock / 2; // Most APB timers run at half system clock

	// Calculate the required prescaler and period for the given microsecond period
	uint32_t ticks_needed = (timer_clock / 1000000) * period_us;
	uint32_t prescaler = 1;
	uint32_t period = ticks_needed;

	// Adjust prescaler and period to stay within 16-bit range
	while (period > 65535)
	{
		prescaler *= 2;
		period = ticks_needed / prescaler;

		if (prescaler > 65535)
		{
			// Cannot achieve this period with 16-bit timer
			return HAL_ERROR;
		}
	}
...
HAL_StatusTypeDef BSP_TIMER_set_frequency_Hz(timer_id_t timer_id, float frequency_hz)
{
	...
	return BSP_TIMER_set_period_us(timer_id, period_us);
}
